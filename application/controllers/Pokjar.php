...
/**
 * Description of Pokjar
 *
 * @author Kent-os
 */
class Pokjar extends CI_Controller{
    
    public function __construct() {
	  parent::__construct();
	  $this->load->model('Pokjar_M');
    }
    ...
    public function index() {
	  ...
	  $data = [
		...
		"nip" => $ssn->nip,
		"base_url" => base_url(),
		"view" => $this->parser->parse('pokjar/v_pokjar', ["flash" => $flash], TRUE)
	  ];
	  $this->parser->parse('template/template', $data);
    }

    public function ListPokjar() {
	  echo $this->Pokjar_M->ListPokjar();
    }

    public function Simpan() {
	  $this->Pokjar_M->Simpan();
    }

    public function Update() {
	  $this->Pokjar_M->Update();
    }

    public function Remove($idPokjar) {
	  $this->Pokjar_M->Remove($idPokjar);
    }
